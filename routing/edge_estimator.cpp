...
double GetSpeedMS(EdgeEstimator::Purpose purpose, Segment const & segment, RoadGeometry const & road)
{
                        ...
                        RoadGeometry const & road, CalcSpeed && calcSpeed)
{
  ...
  double speedMpS = GetSpeedMS(purpose, segment, road);

  static double constexpr kSmallDistance = 0.01;
  if (distance > kSmallDistance && !IsTransit(road.GetHighwayType()))
  {
...
{
  double result = road.GetDistance(segment.GetSegmentIdx()) / GetSpeedMS(purpose, segment, road);
